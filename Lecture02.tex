...
\item The expectation value of the residual deviation should be
  $E(\epsilon)=0$.
\pause\item \textbf{Consequences:} \bfgreen{None}: The Ordinary Least
...
but is no longer efficient} (a ``mild'' error).
...
\pause \item \textbf{Consequences:} a violation has \bfyellow{very
...
since OLS will detect it for you returning a ``0/0 error''

\pause \item If there are only a few more data points than parameters, the
  data specification is OK but the \bfyellow{estimation errors are big}
...
quality
\pause\item \textbf{Consequences:} OLS \bfgreen{detects a perfect
...
S &=&  \left(\vec{y}-\m{X} \vec{\beta}\right)\tr 
 ...
 \visible<2->{\remark{distributivity \& associativity}
...
$\ablpart{}{\vecbeta}(\beta\tr \vec{a})=\vec{a}$ and
$\ablpart{}{\vecbeta}(\beta\tr
