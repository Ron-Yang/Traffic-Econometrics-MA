...
\pause \itemAnswer \colAnswer{Since, as a probability or probability
  density, $L>0$ and the log function is defined and strictly monontonously
  increasing in this range. Since
\bdm
x<y \Leftrightarrow f(x) < f(y)
\edm
if $f(.)$ is strictly monontonously
  increasing and the maximum function is based on the inequality
...
\visible<2->{\tilL(\vecbeta) &=& \sum_{n=1}^N \ln f_n(y_n)
 = \sum_{n=1}^N\left\{
 -\frac{1}{2}(\ln 2\pi+\ln \sigma^2)
 -\left[\frac{(y_n-\vec{\beta}\vec{x}_n)^2}{2\sigma^2}\right]\right\}
 \nonumber\\}
 \visible<3->{&=& -\frac{N}{2}(\ln 2\pi+\ln \sigma^2)
 -\frac{1}{2\sigma^2}(\vec{y}-\m{X}\vec{\beta})\tr(\vec{y}-\m{X}\vec{\beta})}
\edma
\visible<4->{This has the same functional form as the sum of squared
  errors of the 
OLS method and leads to the same estimator!}

\vspace{1em}

\bi
\visible<5->{\itemAsk \colAsk{Why it is possible 
to express $L(\vecbeta)$ as a product, here?}}
\visible<6->{\itemAnswer \colAnswer{Since the random terms $\epsilon_n
    \sim i.i.d 
...
\\[1em]
\visible<3->{\pause \item Probability to predict all the data assuming
...
{\small
\bi
\pause \itemAsk \colAsk{ Show that, in deriving the main ML result
$\tilL=\sum_n\sum_iy_{ni}\ln P_{ni}$, the random utilities need not to
be uncorrelated between alternatives, only between choices}

\pause \itemAnswer \colAnswer{Because of the exclusivity/completeness
  ...
  probability (last expression in the loglikelihood-formula)}
...
\placebox[center]{0.50}{0.84}{$V_{ni}=\beta_1\delta_{i1}+\beta_2T_{ni}$}



\placebox[center]{0.25}{0.25}
 {\figSimple{0.51\textwidth}{figsDiscr/kalLogitBinom_beta1beta2.png}}

\placebox[center]{0.75}{0.25}
 {\figSimple{0.51\textwidth}{figsDiscr/kalProbitBinom_beta1beta2.png}}

\placebox[center]{0.50}{0.65}{\parbox{\textwidth}{
{\footnotesize
\begin{center}
...
\placebox[center]{0.37}{0.44}{
  \figSimple{0.70\textwidth}{\pathDiscrChoice/RC_skript_4alt_r_eng_fProb.png}
}

\placebox[center]{0.85}{0.72}{\parbox{0.3\textwidth}{\small
$
\begin{array}{l}
V_1=\beta_1+\beta_4 r,\\ 
V_2=\beta_2+\beta_5 r,\\
...
\placebox{0.30}{0.45}{\figSimple{0.62\textwidth}
 {figsDiscr/BNL_Entfernung_smallSample_relHaeuf_eng.png}}

\placebox{0.77}{0.45}{\parbox{0.40\textwidth}{\small
\bi
\itemAsk \colAsk{ What would be the modelled ped/bike modal split for
...
\pause \itemAsk \colAsk{ Read off from the $\tilL$ contour plot the parameter of
  ...
  with $P_1/P_2=N_1/N_2=20/32$}
\pause \itemAsk \colAsk{ Motivate the negative correlation between the
  parameter errors} \pause \colAnswer{Make sure that the same fraction choses
  alternative~2}
\ei
}}
