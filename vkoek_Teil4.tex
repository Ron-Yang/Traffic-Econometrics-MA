...
\bfblue{diskrete Entscheidungen}, d.h. man muss sich f\"ur eine aus
...
\bi
\item Zun\"achst wird in der \bfblue{Aktivit\"atenwahl} (fr\"uhmorgens
...
\item In der darauffolgenden \bfblue{Zielwahl} wird entschieden,
\textit{wo} die gew\"ahlten Aktivit\"aten stattfinden sollen
(z.B. in welchen L\"aden eingekauft werden soll) 
\item Danach oder gleichzeitig wird in der \bfblue{Verkehrsmittelwahl}
...
\item Schlie\3lich wird in der \bfblue{Routenwahl} der g\"unstigste
...
Punkt repr\"asentiert. Ferner sind \bfblue{Indifferenzkurven} gleichen
...
der \bfblue{logistische Regression} nutzt, der aber nicht gut
...
normalverteilte Zufallsnutzen, welche zu den \bfblue{Probit-Modellen}
f\"uhren (Abschnitt \ref{sec:Probit}) und extremwertverteilte
(genauer: gumbelverteilte)
Zufallsnutzen, welche den in Abschnitt \ref{sec:Logit} behandelten \bfblue{Logit-Modellen}
...
(\bfblue{Heteroskedastizit\"at}) und sind statistisch begr\"undet
...
verzichten, kommen \bfblue{Generalized
...
wichtiger und anschaulicher Spezialfall der GEV-Modelle sind die 
 \bfblue{Nested-Logit Modelle} (Abschnitt~\ref{sec:nestedLogit}).
...
unterschiedliche \bfblue{Alternativenmengen} $A_n=\{a_{ni}\}$ zur
...
Beispiel des bekannten \bfblue{Red-Bus Blue-Bus Problems}: 
...
einen Satz von \bfblue{generischen Variablen}, sog. \bfblue{Charakteristika}
...
\item Rein personenbezogene Faktoren werden durch
 einen Satz von 
\bfblue{sozio\-\"okono\-mischen Variablen} $\vec{S}_n=\{S_{jn}\}, j=1,
..., J_S$
...
dichotome exogene \bfblue{Dummyvariable} (Pseudovariable) in der
deterministischen  Nutzenfunktion modelliert.
}

\item \bfblue{Externe Variable} wie das Wetter h\"angen weder von der
...
noch \bfblue{konstante Pr\"aferenzen} f\"ur
...
\bfblue{exogenen Einflussfaktoren} $x_{nmi}$ zu\-sammen\-fasst:
\be
\label{xmni}
...
\bfblue{impliziten Zeitwert} (engl. \bfblue{VTTS} f\"ur \bfblue{value of
...
\item Sozio\"okonomische Variable sind per Definition f\"ur alle
  ...
  \bfblue{alternativenspezifische Variable} bzw. Faktoren formuliert
  werden, indem man sie an 
  jeweils eine Alternative \"uber \bfblue{Selektoren} koppelt. Selektoren
...
\item Die Alternative, welche nicht durch Selektoren ausgew\"ahlt
  wird, ist die \bfblue{Referenzalternative}. Die Parameter geben die
...
Regressionsrechnung,  werden durch \bfblue{alternativenspezifische
...
\maintext{Der rationale Entscheider oder \bfblue{Homo Oeconomicus}
...
zu den Regressionsmodellen (Kap. \ref{sec:spez}) wird daher ein \bfblue{Zufallsnutzen}
...
\bdm
U_i=U(\underbrace{\vec{C}_i, \vec{S}}_{\rm bekannt}, 
          \underbrace{\vec{C}'_i, \vec{S}'}_{\rm unbekannt})
...
\bdm
U_i=U(\vec{C}_i \underbrace{+\vec{\epsilon}_i}_{\rm Messfehler}, 
  \vec{S} \underbrace{+\vec{\epsilon}}_{\rm Messfehler})
...
\"uber \bfblue{Instrumentenvariable} (\textit{observables})  $\vec{I}$ messbar, z.B. f\"ur
...
\bi
\item Im \bfblue{Logit-Modell} sind alle obigen Eigenschaften
  erf\"ullt, die Zufallsnutzen sind also \text{i.i.d.} Au\3erdem ist die
  Verteilung durch die  \bfblue{Gumbel-Verteilung} spezifiziert
  (siehe 
  Abschnitt~\ref{sec:Logit}). Manchmal unterscheidet man nach der Zahl der Alternativen:
 \bi
   \item Das \bfblue{Binomiale Logit Modell} beschreibt genau zwei
   Alternativen,
   \item  das \bfblue{Multinomiale Modell}  beschreibt $I\ge 3$ Alternativen.
 \ei

\item Im allgemeinen \bfblue{Probit-Modell} sind die Zufallsnutzen
...
\item Im \bfblue{Nested Logit-Modell} werden bei den gumbelverteilten
...
\item In der Klasse der \bfblue{GEV-Modelle} (\textit{generalized
...
\item Schlie\3lich haben die Zufallsnutzen des \bfblue{Mixed-Logit}
...
\bfblue{Entscheidungs\-wahr\-schein\-lich\-kei\-ten} $P_{ni}$ daf\"ur, dass
...
Ein {\it Vorteil} der Probit-Modells besteht in der
Konsistenz der Herleitung dieser Modelle f\"ur viele praktische
F\"alle:
Nach dem \bfblue{Zentralen
...
normalverteilt, {\it unabh\"angig} von den Verteilungen der einzelnen
...
Der {\it Nachteil} des Probit-Modells ist die f\"ur den multinomialen
...
\bfblue{\mbox{(Typ I-)} Extremwertverteilung}. 
...
Probit-Modelle) \bfblue{translationsinvariant}. Man kann also ohne
...
Probit-Modelle) \bfblue{skaleninvariant}. Dies wurde ebenfalls bereits
...
sogenannte \bfblue{IIA-Eigen\-schaft}. Dieses K\"urzel kommt von der
...
\benum
\item Die \bfblue{Elastizit\"at bez\"uglich Verschiebungen} bzw. die
  \bfblue{Substitutions-Elastizit\"at} 
  (engl.: \emph{mode-choice elasticity})
  gegen\-\"uber der \bfblue{vollen Elastizit\"at}
...
\item Die \bfblue{mikroskopische Elastizit\"at}, welche auf eine
  bestimmte Person und einen bestimmten Flug $n$ bezogen ist,
  gegen\"uber der \bfblue{makroskopischen 
...
\item \bfblue{Eigenelastizit\"aten} $\epsilon_{ii}$ als
  ...
  anderen Flugh\"afen 
  konstant bleiben, im Gegensatz zu \bfblue{Kreuzelastizit\"aten}
...
alternatives Verfahren, die \bfblue{Maximum-Likelihood-Methode}.
...
diese Funktion \bfblue{Likelihoodfunktion} genannt.\footnote{Im
...
Methoden der \bfblue{nichtlinearen Optimierung}. Bekannte Verfahren
sind
\bi
\item das konzeptionell einfachste, aber bei Nichterf\"ullung der
  globalen Konvexit\"at
h\"aufig instabile \bfblue{Newton-Verfahren}
...
\bfblue{Hesse-Matrix} der zweiten 
...
\bfblue{Informationsmatrix} genannt:
\be
\label{HesseL}
...
\bfblue{Merkmalssummen} als Summe der exogenen Faktoren $x_{nmi}$
...
\bfblue{Newton'sche Iterationsverfahren} vorgestellt, welches meist in
...
also die \bfblue{Choice Sets}, auf den Weg von zu Hause in den
...
\bfblue{wahlbasierten Conjoint-Analyse} analy\-siert, was \"aquivalent
...
\ee
manchmal als \bfblue{(Fisher'sche) Informationsmatrix} bezeichnet
...
exakt, deshalb spricht man auch von der \bfblue{asymptotischen
Kovarianzmatrix} der Parametersch\"atzer. 

\item
Harald Cram{\'e}r und Calyampudi Radhakrishna Rao haben einen wichtigen
Satz, die \bfblue{Cram{\'e}r-Rao Ungleichung} hergeleitet: Eine untere
...
\emph{Discrete-Choice} Modelle ist \bfblue{effizient}.  
...
\bfblue{Schwellen} vorgeschlagen: Es wird angenommen, dass es egal ist,
...
eine \bfblue{Sensitivit\"atserh\"ohung} in der N\"ahe von
...
Verhaltensmuster ist auch als \bfblue{loss aversion} bekannt.}
...
Alternative \bfblue{dominant} gegen\"uber den anderen ist: Beeinflusst nur
...
der \bfblue{Modellspezifikation}  der
...
anwendbar, das hei\3t, das einfachere oder \bfblue{restringierte}
Modell $\Mr$ mit $M_r$ Parametern ist im komplexeren \bfblue{unrestringierten} Modell~M
...
\bfblue{Trivialmodell}: Bei quasilinearen stetigen Modellen (Regressionsmodellen)
...
\bfblue{Konstantenmodell}: Bei den quasilinearen Regressionsmodellen
...
\bi
\item \bfblue{Akaike's information criterion}:
  \be \text{AIC}=-2\tilL+2M\frac{n}{n-(M+1)} \ee

\item \bfblue{Bayesian information criterion:}
   \be \text{BIC}=-2\tilL+M\ln n \ee

\item \bfblue{LR-Index} bzw. \bfblue{McFaddens R$2$}:
   \be \rho^2=1-\frac{\tilL}{\tilL\sup{0}} \ee
 
\item \bfblue{korrigierter LR-Index} bzw. \bfblue{adjusted
  likelihood-ratio index} bzw. \bfblue{McFaddens
  korrigiertes R$2$}:
...
\item Bei verschachtelten Modellen, welche der Nullhypothese des
  ...
  \item Ein weiterer Test der Nullhypothese gleicher G\"ute zweier
    ...
    \bfblue{Horowitz-Test}. Dieser wird hier aber nicht weiter
    behandelt.
...
Das bekannteste nichttriviale GEV-Modell ist das \bfblue{Nested-Logit-Modell}
...
\ee
\bi
\item In der \"ubergeordneten Entscheidung wird das \bfblue{Nest} $l$ gew\"ahlt, beispielsweise beim
...
\item F\"ur $\lambda_l \to 0$ (\bfblue{sequentielles
...
Zwei weitere h\"aufig benutze GEV-Varianten sind die \bfblue{Paired
  Combinatorial Logit (PCL)} und \bfblue{Generalized Nest Logit (GNL)}
...
Die Klasse der \bfblue{Mixed-Logit-Modelle}  enth\"alt stochastische
...
die Dichtefunktion bzw. \bfblue{mixing function} $f(\vecbeta)$
gegeben ist.}
%
In der Praxis gibt
...
\bi
\item [I] \bfblue{Taste-Variation}, also unterschiedliche Vorlieben f\"ur
  jede Person (\emph{Kenneth E. Train}, Kap. 6.2). Dieser
  Interpretation liegt auch der Name  dieser
  Modellklasse zugrunde,
\item [II] \bfblue{verallgemeinerte Zufallsnutzen}, welche nicht iid Gumbel sind
...
\item Durch einen ``Skalentrick'' kann man
  jedes beliebige \bfblue{Random-Utility Model} 
...
Der begriff \bfblue{Panel} bezieht sich, wie \"ublich, auf die mehrfache Befragung
...
soll, berechnen wir den \bfblue{erwarteten Nutzen} (\emph{expected
