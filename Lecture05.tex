...
\item Example: $H_0$: ``tomorrow is nice weather'' 
\bi
\item $P(H_0)$: a-priori probability before hearing the
  weather forecast (or the general probability 
  based on climate tables)
\item $B$: tomorrow's weather forecast 
$B\in \text{will be nice}, \text{not nice}\}$
\item $P(H_0|B)$: a-posteriori probability 
  after hearing the forecast
\ei
\item Relation to classical frequentist's statistics: Known are some
  observation $B$ and conditional probability $P(B|H_0)$ that can be
  expressed in terms of $p$. Want $P(H_0|B)$
\item Four scaling possibilities
\bi
\item[(i)] discrete $\beta$ and $\hatbeta$ (e.g., Covid-19 test)
\item[(ii)] discrete $\beta$ and continuous $\hatbeta$ (e.g., map-matching)
\item[(iii)] continuous $\beta$, discrete observation ($H_0$ rejected
  or not) 
\item[(iv)] continuous sought-after quantity
...
\bi
\item $H_0$: person is infected; $B$: person is
  tested positive
\item Known: 
\bi
\item[$\bullet$] Sensitivity $P(B|H_0)=\unit[95]{\%}$
\quad $P(\bar{B}|H_0)=\unit[5]{\%}$
\item[$\bullet$] Specificity $P(\bar{B}|\bar{H_0})=\unit[99]{\%}$,
\quad $P(B|\bar{H_0})=\unit[1]{\%}$
\item[$\bullet$] Incidence $P(H_0)=\unit[5]{\%}$
\ei
\ei
}
\vspace{-1em}
...
{\small
\begin{align*}
\text{Bayes: pos. rate: \ }P(B) \!&= P(B|H_0)P(H_0)+P(B|\bar{H_0})P(\bar{H_0})
%=0.99 * 0.05 + 0.05*0.95
=\unit[9.7]{\%},\\
...
$
\item Continuous measurement $\hatbeta$  with a given 
  distribution of density 
$g(\hatbeta \ | \ \beta=\beta_j)=f(\hatbeta-\beta_j)$
\bi
\itemAsk \colAsk{What is the meaning of $f(.)$?}
\ei

\item Assume $H_0: \beta=\beta_{j_0}$ with $\beta_{j_0}\in \{\beta_j\}$
...
\placebox{0.5}{0.05}{$p(H_0)=\frac{\text{flow freeway}}
{\text{flow freeway}+\text{flow road}}=0.8$}
...
\item The quantity $\beta$ has the a-priori distribution
  density $h(\beta)$\\[1ex]
\item Unlike discrete quantities/parameters, $H_0$ meeds to be an
  interval instead of a point \colAsk{(why?)} $\Rightarrow P(H_0)$ and
  $P(B|H_0)$ are integrals over the values of $H_0$\\[1ex]
\item Probability for $H_0$ based on measurements lying in the extreme
...
%###################################################
\frame{\frametitle{Continuous parameter II: Inference for a given measurement}
%###################################################
\vspace{0.5em}
...
\item $\beta$ has the a-priori distribution
  density $h(\beta)$\\[1ex]
\item The estimation error $\hatbeta-\beta$ is independent from $\beta$ (as in the OLS estimator under
...
\item Unbiased estimator $ \hatbeta \sim N(\beta,\sigma_b^2)$
\item Null hypothesis $H_0$: $\beta \le \beta_0$\\
$\Rightarrow
...
\item Bayesian inference for $H_0$ under the observation
  $\hatbeta=b$ (long calc.):
...
\item When expressing the observation in terms of the $p$ value,
...
\item If $\sigma_b^2 \ll \sigma_{\beta}^2$ and $H_0$ is an
...
{\small
\bi
\itemAsk \colAsk{Show that, if the variance of the prior distribution
...
P(H_0|\hatbeta) &\to & \Phi\left(\frac{\beta_0-b}{\sigma_b}\right)\\
 ...
 \stackrel{\text{def quantile}}{=}\uu{p}
\edma
}
...
\item New investigation: $\hatbeta=\unit[(26 \pm 3)]{\%}$
\ei
...
\item Discrete variables and continuous measurements: 
\bi
\item[$\bullet$] If the measuring
  uncertainty is larger than the distance between possible discrete
  true values, then the \red{a-priori probability} matters
\item[$\bullet$] If the uncertainty is much smaller, then the
  \red{closest distance} to 
  the measurement matters
\item[$\bullet$] The \red{$p$ value is completely mislading}, even
  for bimodal 
  continuous variables (vehicle not exactly in the middle of the right lane)
\ei\vspace{1ex}

\item Continuous variables and measurements:
\bi
\item[$\bullet$] The $p$ value only gives a good estimate for the posterior
...
\item[$\bullet$] If the measuring  uncertainty is much larger than the prior
