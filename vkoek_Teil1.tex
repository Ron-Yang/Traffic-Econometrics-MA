...
\date{\Large Wintersemester 2018/19}
\publishers{\small\copyright{} 2008-2018 Martin Treiber. 
...
% gern erteilt wird
%(email an {\tt treiber\symbol{64}vwi.tu-dresden.de}). Auf die
...
Das Wort \bfblue{\"Okonometrie} hat seinen Ursprung von der \"Okonomie
...
\bfblue{empirische} (also auf Messung, Beobachtung und Erfahrung beruhende)
\textit{\"Uberpr\"ufung}. 
Bei den durch Messung und Beobachtung gewonnen Daten handelt es sich
fast immer um \bfblue{statistische Daten}, also um eine
...
\bfblue{mathematische Modelle} auch die Auswertung der
Daten durch \bfblue{statistische Modelle} eine Rolle. Das
...
\maintext{Die \bfblue{Verkehrs\"okonometrie} umfasst die Gesamtheit
...
Die $Y_k$ sind die \bfblue{erkl\"arten Variablen}.  Eine solche
Variable wird
bisweilen auch als \bfblue{Explanandum} (lat. ``zu erkl\"arende''
bzw. erkl\"arte Variable), als
\bfblue{endogene Variable} (Griechisch: von innen heraus, also aus dem
Modell kommend) oder gem\"a\3 allgemeinen mathematischen Sprachgebrauch als
\bfblue{abh\"angige Variable} bezeichnet. In der Systemtheorie
...
von
\bi
\item \bfblue{Eingleichungsmodelle}: Nur eine endogene Variable
$Y_1=Y$,
\item \bfblue{Mehrgleichungsmodelle}: Mehrere endogene  Variable $Y_k$, die
miteinander gekoppelt sind (vgl. Abb.~\ref{fig:flussdiag} rechts),
\item \bfblue{Mehreren Eingleichungsmodelle}: Mehrere ungekoppelte
...
Die Gr\"o\3en $x_m$, $m=1, \ldots, M$ sind die  \bfblue{erkl\"arenden
Variablen}, welche auch als  
\bfblue{Explanans} (lat. ``erkl\"arend''), \bfblue{exogene Variable}
 (Griechisch: von au\3en kommend) 
oder allgemein als
\bfblue{unabh\"angige Variable} bezeichnet werden.
...
\bi
\item \bfblue{univariate Modelle}: $M=1$,
\item \bfblue{multivariate Modelle}\footnote{Streng genommen sind die
...
Die Gr\"o\3en $\beta_j$, $j=0, \ldots, J$ sind die \bfblue{Modellparameter}. Im
...
\bfblue{Sch\"atzung} bzw. \bfblue{Modellkalibrierung} und einer
...
\bfblue{nicht erkl\"arte Anteile} welche meist durch Zufallsvariablen
...
sonst \bfblue{fehlspezifiziert} ist und falsche Aussagen liefert
...
\bi
\item Das \bfblue{\"okonometrische Modell im engeren Sinn} 
...
\item Die \bfblue{Systemgleichungen}, manchmal auch als
\bfblue{Messgleichungen} bezeichnet, beschreiben die
...
\bi
\item \bfblue{Lineare Modelle}: Hier h\"angen die endogenen Variablen
...
\item \bfblue{Reduzible nichtlineare Modelle}: Zwar h\"angt hier
die endogene 
...
\item \bfblue{Irreduzible nichtlineare Modelle}: Hier h\"angt die endogene
...
\bfblue{qualitative} bzw. \bfblue{nominalskalierte Variablen} handelt,
...
\bfblue{Modell beschr\"ankten Wachstums} mit der Zeit $x$ als exogenen
...
\item \bfblue{Quasilineare} bzw. \bfblue{parameterlineare Modelle}:
...
\bfblue{Deterministische} \textit{vs.} \bfblue{stochastische}
Modelle. 
...
\bfblue{Entscheidungsmodelle} haben diskrete
bzw. qualitative/nominalskalierte endogene Variablen, w\"ahrend
\bfblue{kontinuierliche Modelle} stetige sowie verh\"altnisskalierte
...
durch kardinalskalierte \bfblue{Pseudovariablen} der Art 1=Maurer, 2=Schreiner,
...
\bi
\item Bei \bfblue{verketteten Modellen}
...
\item Bei \bfblue{r\"uckgekoppelten Modellen} koppeln die endogenen
Variablen   
...
\item Dynamische \textit{vs.} statische Modelle: \bfblue{dynamische
...
kennen lernen, das \bfblue{dynamische Input-Output-Modell} (Abschnitt
\ref{sec:IOM}). 
...
\bfblue{mikroskopisch}, betrachten also jede Entscheidung einzeln. Damit
...
\bfblue{makroskopische Hernagehensweise}
