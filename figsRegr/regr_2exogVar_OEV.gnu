...
# x1=Preis
# x2=Geschw.
# y=Zahl OEV-Fahrten/Jahr/Person
...
#########################################################
print ""
print "Mittelwerte, Varianz- und Kovarianzmatrizen"
print "====================================="
#########################################################
...
print "Korrelationen und deskr. Bestimmtheitsma3: r12=",r12," B=r^2(y, haty)=",B
print "deskriptive erklaerte Varianz s_hatyhaty=",s_hatyhaty
print "deskr. nichterkl. Varianz (Var.-Additionsregel) s_epseps=",s_epseps
...
#########################################################
print ""
print "Geschaetzte Kovarianzmatrix der beta_j  (2 exogene Var x1,x2)"
print "======================================"
#########################################################
...
#########################################################
print ""
print "Konfidenzintervalle zu alpha=0.05"
print "======================================"
#########################################################
...
###### m1=Modell mit nur x1, m2=Modell mit nur x2 #########
...
print "\nHalbe Breiten der KI und KI selbst bei Modellen mit nur x1 bzw. x2:"
...
#######################################
print "plotting OEV_f_gaussStudent.eps"
set out "OEV_f_gaussStudent.eps"
#######################################
set noparam
xnormmin=-2.
xnormmax=6.5
...
set ylabel "Dichtefunktion f_z(z) bzw. f_t(t)"
set auto y
set key

plot\
 gauss(0,1,x) t "Standardnormalverteilung" w l ls 1,\
 student(x,1) t "Student-Verteilung mit {/Symbol n}=1 FG" w l ls 2,\
 student(x,2) t "Student-Verteilung mit {/Symbol n}=2 FG" w l ls 3
...
set ylabel "Verteilungsfunktion F_z(z) bzw. F_t(t)"
set auto y
set key center right
set param

plot[x=xnormmin:xnormmax]\
 x,phi(x) t "Standardnormalverteilung" w l ls 1,\
 x,studentCum(x,1) t "Student-Verteilung mit {/Symbol n}=1 FG" w l ls 2,\
 x,studentCum(x,2) t "Student-Verteilung mit {/Symbol n}=2 FG" w l ls 3,\
 x,0.95 t "" w l ls 99,\
...
#######################################
print "plotting OEV_f_hatbeta1.eps"
set out "OEV_f_hatbeta1.eps"
#######################################

set title "Konfidenzintervall zu H_0: {/Symbol b}_1 und {/Symbol s}  wie gesch\344tzt und {/Symbol a}=0.05"
tmin=-1.5*t_nm3_0975
tmax=2.1*t_nm3_0975
...
set ylabel "Dichtefunktion f(hat {/Symbol b}_1)"
set param
...
plot [t=tmin:tmax]\
  beta1+t*sigbeta1,studentNoNorm(beta1+t*sigbeta1,beta1,sigbeta1,n-3)\
    t "Dichte" w l ls 2,\
  beta1-dbeta1 + (t-tmin)/(tmax-tmin)*2*dbeta1,0 t "Konfidenzintervall" w l ls 21
...
set ylabel "Verteilungsfunktion F(hat {/Symbol b}_1)"
...
#######################################
print "plotting OEV_f_hatbeta2.eps"
set out "OEV_f_hatbeta2.eps"
#######################################
...
set ylabel "Dichtefunktion f(hat {/Symbol b}_2)"
plot [t=tmin:tmax]\
  beta2+t*sigbeta2,studentNoNorm(beta2+t*sigbeta2,beta2,sigbeta2,n-3)\
    t "Dichte" w l ls 2,\
  beta2-dbeta2 + (t-tmin)/(tmax-tmin)*2*dbeta2,0 t "Konfidenzintervall" w l ls 21
...
#######################################
print "plotting OEV_f_studentTestvar.eps"
set out "OEV_f_studentTestvar.eps"
#######################################

set title "Konfidenzintervall zu H_0: {/Symbol b}_{1/2} und {/Symbol s} wie gesch\344tzt unter  {/Symbol a}=0.05"

set key right top
set xlabel "Testvariable t"
set xrange [tmin:tmax]
set ylabel "Dichtefunktion f(t)"
plot [t=tmin:tmax]\
  t,student(t,n-3) t "Dichte" w l ls 2,\
  -t_nm3_0975 + (t-tmin)/(tmax-tmin)*2*t_nm3_0975,0 t "Konfidenzintervall" w l ls 21

#######################################
print "plotting OEV_f_student_KI.eps"
set out "OEV_f_student_KI.eps"
#######################################
tmin=-4
tmax=4
set xrange [tmin:tmax]
...
plot [t=tmin:tmax]\
  t,student(t,2) t "Dichte" w l ls 2,\
  -tQuantil(0.95,2) + (t-tmin)/(tmax-tmin)*2*tQuantil(0.95,2),0 t "Konfidenzintervall" w l ls 21
...
set ylabel "Verteilungsfunktion F(t)"
...
#######################################
print "plotting OEV_scatterplot_x1x2.eps"
set out "OEV_scatterplot_x1x2.eps"
#######################################

set xlabel "Fahrpreis x_1 (Euro)"
set ylabel "Geschwindigkeit x_2 (km/h)"
set nokey
...
#######################################
print "plotting OEV_scatterplot_x1y.eps"
set out "OEV_scatterplot_x1y.eps"
#######################################

set xlabel "Fahrpreis x_1 (Euro)"
set ylabel "Fahrgastzahlen y (Fahrten/Person/Jahr)"
set key
...
plot[x1=x1datamin:x1datamax]\
  ...
  x19,y9 t "" w p ls 1,\
  x10,y0 t "" w p ls 1,\
  x1, ybar+s1y/s11*(x1-x1bar) t "Einfachregression(x_1)" w l ls 2,\
  x11,haty(x11,x21) t "Mehrfachregression(x_1,x_2)" w p ls 3,\
...
#######################################
print "plotting OEV_scatterplot_x2y.eps"
set out "OEV_scatterplot_x2y.eps"
#######################################

set xlabel "Geschwindigkeit x_2 (km/h)"
set key
...
plot[x2=x2datamin:x2datamax]\
  ...
  x29,y9 t "" w p ls 1,\
  x20,y0 t "" w p ls 1,\
  x2, ybar+s2y/s22*(x2-x2bar) t "Einfachregression(x_1)" w l ls 2,\
  x21,haty(x11,x21) t "Mehrfachregression(x_1,x_2)" w p ls 3,\
...
#######################################
print "plotting OEV_f2_hatbeta1_hatbeta2.eps"
set out "OEV_f2_hatbeta1_hatbeta2.eps"
#######################################
...
set pm3d
unset clabel
set title "2d-Dichte hat {/Symbol b}_j unter H_0: {/Symbol s} und {/Symbol b}_j wie gemessen"
print "rbeta12=",rbeta12
...
splot[x=xmin:xmax][y=ymin:ymax]\
  x,y,gauss2d(x-beta1,y-beta2,sigbeta1,sigbeta2,rbeta12)\
   t "H_0 - Konfidenzregion F-Test" w l ls 7

set surface
set key at screen 0.7,0.26
splot[t=0:1]\
 beta1-dbeta1,beta2-dbeta2+2*t*dbeta2,100+0.01*t\
      t "H_0 - Konfidenzregion t-Test" w l ls 5,\
...
#######################################
print "plotting OEV_hyperbel_M1.eps"
set out "OEV_hyperbel_M1.eps"
#######################################
...
set ylabel "y (Fahrten/Jahr/Person)"
set yrange [ydatamin:ydatamax]
...
splot [x1=x1min:x1max][y=ydatamin:ydatamax]\
  x1,y,fdens_haty_m1(y,x1) t "Dichte f(hat y|x_1)" w l ls 98
...
splot [x1=x1min:x1max][y=ydatamin:ydatamax]\
  x1,y,Fdens_haty_m1(y,x1) t "2.5% und 97.5%-Quantile" w l ls 7

set surface
set key at screen 0.8,0.97
splot\
  x11,y1,101 t "Daten" w p ls 1,\
...
#######################################
print "plotting OEV_scatter3d.eps"
set out "OEV_scatter3d.eps"
#######################################
set nogrid
unset colorbox
