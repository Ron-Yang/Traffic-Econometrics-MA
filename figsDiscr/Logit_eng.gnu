...
#########################################################
# Logit-Wahrsch. bei drei Alternativen als f(V1, V2) ; V3=0 und Var(eps)=pi/sqrt(6)
#########################################################


P1trinom(V1,V2)=1./(exp(-V1)+exp(V2-V1)+1)
...
#################################
set out "pLogitTrinom_V1V2.eps"
print "plotting pLogitTrinom_V1V2.eps"
#################################
...
set label 1 "V_3=0" at screen 0.4,1.31 front
set label 2 "P_1" at screen 1.3,1.31 front
set xlabel "V_1-V_3 (Einheiten {/Symbol s_e})"
set xrange [V1min:V1max]

set ylabel "V_2-V_3 (Einheiten {/Symbol s_e})"
set yrange [V2min:V2max]
unset cntrparam
#set cntrparam levels incr 0,0.1,1
set cntrparam levels discrete 0.05,0.1,0.15,0.2,0.245,0.25,0.255,0.3,0.35,0.4,0.45,0.495,0.5,0.505,\
0.55,0.6,0.65,0.7,0.745,0.75,0.755,0.8,0.85,0.9,0.95

scalefact=pi/sqrt(6)
splot P1trinom(scalefact*x,scalefact*y) w l lt 6 lw 4 

unset label 1
unset label 2

#################################
set out "kalLogitBinom_beta1beta2.eps"
print "plotting kalLogitBinom_beta1beta2.eps\n"
#################################
...
# Logit-Wahrscheinlichkeit, falls Nutzenfunktion in Einheiten von sqrt(2*sigeps)

P1binom(dV)=1./(1+exp(-dV))
P2binom(dV)=1.-P1binom(dV)
...
# Konstanter Binomialkoeffizient-Term

lnfac(x)=(x<50) ? log(gamma(x+1)) : x*log(x)-x

logB(ni, y1i)= lnfac(ni)-lnfac(y1i)-lnfac(ni-y1i)  
logL_BinomKoeff=logB(ni, y11)+logB(ni, y12)+logB(ni, y13)\
  +logB(ni, y14)+logB(ni, y15)+logB(ni, y16)
print "logL_BinomKoeff=",logL_BinomKoeff



print "Vergleich Logitmodell-Daten mit geschaetzten Parametern:"
b1=-0.7
b2=-0.08
...
set param
splot[x=beta1min*scalefact:beta1max*scalefact][y=beta2min*scalefact:beta2max*scalefact]\
  x/scalefact,y/scalefact,logL(x,y) w l lt 6 lw 4 


#################################
set out "kalLogitTrinom_beta1beta2.eps"
print "\nplotting kalLogitTrinom_beta1beta2.eps\n"
#################################
...
splot[x=beta1min*scalefact:beta1max*scalefact][y=beta2min*scalefact:beta2max*scalefact]\
  x/scalefact,y/scalefact,logLtri(x,y) w l lt 6 lw 4 
